...
-- =============================================================================
-- BRAND SOUL CACHE COLUMNS (Pieza 5: Brand Soul LLM Integration)
-- =============================================================================
-- These columns store the generated Brand Soul HTML document and cache metadata:
-- - soul_html: the generated HTML document (cached)
-- - brain_hash: SHA256 hash of brain state for cache invalidation
-- - soul_generated_at: timestamp when the document was last generated
--
-- Migration version: 2025-01-23-soul-cache
-- =============================================================================

-- Add soul_html column (cached HTML document)
do $$
begin
    if not exists (
        select 1 from information_schema.columns
        where table_name = 'brand_brains'
        and column_name = 'soul_html'
    ) then
        alter table brand_brains add column soul_html text;
        comment on column brand_brains.soul_html is 'Cached Brand Soul HTML document. Generated by LLM, validated for citations.';
    end if;
end
$$;

-- Add brain_hash column (for cache invalidation)
do $$
begin
    if not exists (
        select 1 from information_schema.columns
        where table_name = 'brand_brains'
        and column_name = 'brain_hash'
    ) then
        alter table brand_brains add column brain_hash text;
        create index if not exists brand_brains_brain_hash_idx on brand_brains(brain_hash);
        comment on column brand_brains.brain_hash is 'SHA256 hash of brain state. Used to invalidate cache when brain changes.';
    end if;
end
$$;

-- Add soul_generated_at column (timestamp of last generation)
do $$
begin
    if not exists (
        select 1 from information_schema.columns
        where table_name = 'brand_brains'
        and column_name = 'soul_generated_at'
    ) then
        alter table brand_brains add column soul_generated_at timestamptz;
        comment on column brand_brains.soul_generated_at is 'Timestamp when Brand Soul was last generated. Used for debugging and analytics.';
    end if;
end
$$;
