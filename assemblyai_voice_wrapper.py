...
class AssemblyAISpeechEngine:
    ...
    def start_realtime_transcription(
        ...
        def handle_turn(client, event):
            ...
            if getattr(event, "end_of_turn", False) or getattr(event, "is_final", False):
                ...
                if on_partial_callback:
                    partial_text = getattr(event, "transcript", "") or getattr(event, "text", "")
                    if partial_text:
...
# ========================================================================================
# DEMO RÁPIDA DE PRUEBA LOCAL
# ========================================================================================
if __name__ == "__main__":
    print("==================================================================")
    print("SINGULARITYOS - TEST DE WRAPPER ASSEMBLYAI VOICE ENGINE")
    print("==================================================================")

    # 1. Instanciar motor STT
    engine = AssemblyAISpeechEngine()

    # Simulación de palabras transcritas con timestamps para probar el detector de silencios
    engine.accumulated_words = [
        WordTimestamp("THIS", 0, 400, 0.99),
        WordTimestamp("IS", 450, 900, 0.98),
        # Bacha de silencio de 600ms entre 900ms y 1500ms
        WordTimestamp("ESTE", 1500, 1800, 0.99),
        WordTimestamp("ES", 1820, 2000, 0.99),
        WordTimestamp("EL", 2020, 2100, 0.99),
        WordTimestamp("FUTURO", 2150, 2800, 0.99),
    ]

    silences = engine.detect_silences_and_dead_air(min_silence_ms=350)
    print(f"\n[Dead-Air Trimmer]: Detectados {len(silences)} silencios a eliminar:")
    for s in silences:
        print(f"   -> Silencio de {s['duration_ms']}ms entre '{s['after_word']}' y '{s['before_word']}'")

    captions = engine.export_hormozi_captions_json()
    print(f"\n[Hormozi Captions JSON]:\n{json.dumps(captions[:3], indent=2)}")
    print("\n✓ Wrapper operativo y listo para conectar con FastAPI y Remotion.")
